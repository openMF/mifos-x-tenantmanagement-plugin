...
public class TenantManagementReadService {
        ...
        if (search != null && !search.isBlank()) {
            ...
            where.append(
                    " and (lower(t.identifier) like ? escape '!' or lower(t.name) like ? escape"
                            + " '!') ");
            final String term =
                    "%" + escapeLikeWildcards(search.trim().toLowerCase(Locale.ROOT)) + "%";
            arguments.add(term);
            arguments.add(term);
        }
        if (status != null) {
     ...
     *
     * <p>The escape character itself is escaped first, or escaping {@code %} would then re-escape
     * the {@code !} that had just been introduced.
     */
    ...
    public TenantData retrieveOne(final Long id) {
        try {
            return this.jdbcTemplate.queryForObject(
                    "select " + TenantRowMapper.SELECT_SCHEMA + " where t.id = ?",
                    new TenantRowMapper(),
                    id);
    ...
    public boolean existsByIdentifier(final String identifier) {
        final Integer count =
                this.jdbcTemplate.queryForObject(
                        "select count(*) from tenants where identifier = ?",
                        Integer.class,
                        identifier);
     ...
     *
     * <p>Prefers the registry's own {@code timezones} table, so an installation that has curated
     * that list keeps control of what administrators may choose. That table ships empty in some
