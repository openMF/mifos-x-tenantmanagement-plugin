...
public final class TenantRowMapper implements RowMapper<TenantData> {
    ...
    public static final String SELECT_SCHEMA =
            " t.id as id, t.identifier as identifier, t.name as name, t.timezone_id as timezoneId,"
                + " t.status as status, t.description as description, t.contact_email as"
                + " contactEmail, t.joined_date as joinedDate, t.created_date as createdDate,"
                + " t.lastmodified_date as lastModifiedDate, ts.id as connectionId, ts.schema_name"
                + " as schemaName, ts.schema_server as schemaServer, ts.schema_server_port as"
                + " schemaServerPort, ts.schema_username as schemaUsername,"
                + " ts.schema_connection_parameters as schemaConnectionParameters, ts.auto_update"
                + " as autoUpdate from tenants t left join tenant_server_connections ts on"
                + " t.oltp_id = ts.id ";
    ...
    public TenantData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
        return new TenantData(
                ...
                // Null for a value this enum does not name - a hand edit or corruption. Reporting
                // it
    ...
    /**
     * @return the tenant's connection, or null when the outer join matched no row - possible
     *     because the join is a left join, so a registry in an inconsistent state still lists its
     *     tenants instead of failing outright
     */
