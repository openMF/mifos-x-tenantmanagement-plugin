 ...
 * own factory (reached via {@link CoreTenantLiquibaseFactory}), then each plugin changelog exactly
 * as its startup bean applies it.
 */
@Service
@Slf4j
public class TenantSchemaMigrationService {
     ...
     *
     * <p>This plugin owns no per-tenant tables, but the tenants it creates must still receive the
     * tables of every other plugin installed alongside it. The default covers the Mifos X plugins
     * that own tenant tables - self-service and savings - and entries whose plugin is not installed
     * are skipped, so the default is safe on an installation running neither.
     *
     ...
     *
     * <p><strong>Order mirrors startup.</strong> Self-service runs before savings because that is
     * the order the startup beans were observed to run in, so a new tenant goes through the same
     * sequence as every other tenant. It is not forced by the one table the two share: both
     * changelogs create {@code m_selfservice_office_service} behind a {@code not tableExists}
     * precondition, so either order is safe for that table. Keeping the startup order guards
     * against cross-plugin dependencies that are not guarded that way.
     *
     * <p><strong>Strings must match the startup beans exactly.</strong> Liquibase identifies a
     * changeset partly by the path it was loaded from. Using the same {@code classpath:/...}
     * strings as the startup beans means the next startup finds every changeset already applied,
     * instead of re-applying them under a different identity and failing on tables that already
     * exist.
     */
    ...
    private final CoreTenantLiquibaseFactory liquibaseFactory;
    ...
    public TenantSchemaMigrationService(
            ...
            final CoreTenantLiquibaseFactory liquibaseFactory,
     ...
     *
     * <p>The tenant is re-read from the registry rather than passed in, so the connection details
     * and the encrypted credentials come back through core's own mapping - including the master
     * password hash that {@link TenantDataSourceFactory} checks before it will open a datasource.
     *
     ...
     * carrying Flyway metadata - is deliberately absent: that exists to adopt a pre-1.6
     * installation, and a schema this service creates is always empty.
     *
