...
public class TenantManagementDataValidator {
     ...
     * tenant could be pointed at an engine's own databases and have Fineract's migrations run
     * inside them.
     */
     ...
     *
     * <p>Not an attempt at RFC 5322 - this address is contact metadata that is displayed, never
     * used to authenticate or to route mail, so catching obvious typos is the whole job.
     */
     ...
     *
     * <p>The digit pattern alone admits 65536-99999, which would pass validation and then fail as
     * an unreachable database - reported as a connection problem instead of the input mistake it
     * is. Blank and non-numeric values are left to the checks that already report them.
     */
                        ...
                        () -> {
                            validator
                                    ...
                                    .failWithCode(
                                            ...
                                            String.join(", ", TenantStatus.names()));
                            return fallback;
                        });
    }
