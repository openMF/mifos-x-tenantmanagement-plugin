 ...
 *
 * <p>Ordered ahead of Fineract's catch-all {@code /api/**} chain - the same way the self-service
 * plugin claims {@code /v1/self/**} - so it needs no change to core.
 *
...
public class TenantManagementSecurityConfiguration {
     ...
     * it declares. Left unordered, this chain and Fineract's catch-all {@code /api/**} chain would
     * be tried in whatever order the beans happened to register, and requests could reach tenant
    ...
    public SecurityFilterChain tenantManagementSecurityFilterChain(
        ...
        // Spring Security 7 takes the user lookup in the constructor; the encoder is still set
        // afterwards. Delegating, so a stored {bcrypt} hash is matched by prefix.
        final DaoAuthenticationProvider provider =
                new DaoAuthenticationProvider(username -> toUserDetails(masterUserStore, username));
                ...
                .authorizeHttpRequests(
                        auth ->
                                auth
                                        ...
                                        .requestMatchers(
                                                HttpMethod.OPTIONS, TENANT_ADMINISTRATION_PATHS)
                                        .permitAll()
                                        .anyRequest()
                                        .hasRole(TenantMasterAccess.SUPER_MASTER_ROLE));

        // The administration UI runs in a browser on another origin, as the web app does,
        // so this chain honours the platform's CORS configuration.
