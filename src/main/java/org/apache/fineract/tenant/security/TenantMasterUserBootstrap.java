...
public class TenantMasterUserBootstrap implements InitializingBean {
    ...
    /**
     * Shortest bootstrap password accepted. A master user controls every tenant on the
     * installation, so a guessable one is refused outright rather than stored.
     */
    ...
    public TenantMasterUserBootstrap(
            ...
            @Value("${fineract.tenant-management.bootstrap-master-username:}")
                    final String username,
            @Value("${fineract.tenant-management.bootstrap-master-password:}")
    ...
    public void afterPropertiesSet() {
        if (username.isEmpty() || password.isEmpty()) {
            if (store.count() == 0) {
                log.warn(
                        "No tenant management master user exists and none is configured;"
                                + " /v1/admin/tenants will refuse every request until"
                                + " fineract.tenant-management.bootstrap-master-username and"
                                + " bootstrap-master-password are set");
            }
            return;
        }
        ...
        if (store.findByUsername(username).isPresent()) {
            log.info(
                    "Master user [{}] already exists; bootstrap configuration left unapplied",
                    username);
            return;
        }
        ...
        if (password.length() < MINIMUM_PASSWORD_LENGTH) {
            ...
            log.error(
                    "Bootstrap password for master user [{}] is shorter than {} characters; no"
                            + " master user was created",
                    username,
